    ...
    <main className="min-h-screen bg-bone">
      ...
      {requests.length > 0 ? (
        <section className="mx-auto max-w-5xl px-6 pt-10">
          <Eyebrow>greatimmob.com</Eyebrow>
          <h2 className="mt-2 font-display text-3xl">Demandes de réservation</h2>
          <div className="mt-4 divide-y divide-mist border border-mist bg-white">
            {requests.map((b) => (
              <div key={b.id} className="flex flex-wrap items-center justify-between gap-3 px-5 py-4">
                ...
                <div className="flex gap-2">
                  <form action={`/api/bookings/${b.id}/confirm`} method="post">
                    <button className="rounded-gi bg-ink px-3 py-2 text-xs text-bone">Confirmer</button>
                  </form>
                  <form action={`/api/bookings/${b.id}/decline`} method="post">
                    <button className="rounded-gi border border-mist px-3 py-2 text-xs">Refuser</button>
                  </form>
                </div>
              </div>
            ))}
